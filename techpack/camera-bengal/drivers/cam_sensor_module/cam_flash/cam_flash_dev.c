...
/* Spes flashlight by muralivijay@github */
#ifdef CONFIG_CAMERA_FLASH_SPES
	flash_en = of_get_named_gpio(gnode, "qcom,flash-gpios", 0);
	flash_now = of_get_named_gpio(gnode, "qcom,flash-gpios", 1);

	if (gpio_is_valid(flash_en) && gpio_is_valid(flash_now)) {
		mgpio_flash_led.flash_en = flash_en;
		mgpio_flash_led.flash_now = flash_now;
		CAM_INFO(CAM_FLASH,
			"Using flash GPIOs: flash_en=%d flash_now=%d",
			flash_en, flash_now);
	} else {
		CAM_INFO(CAM_FLASH,
			"Ignoring flash node without valid GPIOs: flash_en=%d flash_now=%d",
			flash_en, flash_now);
	}
#endif
/* Spes flashlight by muralivijay@github */
